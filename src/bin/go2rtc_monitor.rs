...
fn main() -> Result<()> {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        loop {
            ...
            let chunk = (sleep_for - elapsed).min(Duration::from_millis(gm_cfg.min_tick_ms.max(50)));
            thread::sleep(chunk);
            let new_cfg = config.read().monitors.go2rtc_monitor.clone();
            if Duration::from_millis(new_cfg.poll_interval_ms) != poll_interval
                || new_cfg.min_tick_ms != gm_cfg.min_tick_ms
            { break; }
        }
    }
